...
def cutover(state):
    ...
    try:
        ...
        _run(command+['up','-d','--no-build','--wait','--wait-timeout','180','cliproxy-api','cliproxy-monitor','chatgpt-speech','hermes'],env=env,cwd=root)
        ...
        _run(command+['restart','cliproxy-api','chatgpt-speech','hermes','cliproxy-monitor'],env=env,cwd=root)
        _run(command+['up','-d','--no-build','--wait','--wait-timeout','180','cliproxy-api','cliproxy-monitor','chatgpt-speech','hermes'],env=env,cwd=root)
        ...
        if changed:
            try:
                ...
                _run(command+['up','-d','--no-build','--wait','--wait-timeout','180','cliproxy-api','cliproxy-monitor','chatgpt-speech','hermes'],env=rollback_env,cwd=root)
