...
const capture=startStorageCapture(services,config),server=storageServer(services,config,call,capture.status);
...
for(const signal of ['SIGINT','SIGTERM'])process.on(signal,()=>{
  ...
  void Promise.all([closed,capture.stop()]).then(()=>stores.close()).catch(()=>{process.exitCode=1;});
});
