...
const SECRET_KEYS = ["AI_API_KEY", "TELEGRAM_BOT_TOKEN"];
...
export function createSetupRoutes(envStore: EnvStorePort): SetupRoutes {
  return {
    ...
    getStatus: async () => {
      const presence = await envStore.presence([...SECRET_KEYS, "TELEGRAM_WEBHOOK_URL", "LOCAL_ENCRYPTION_SECRET"]);
      const values = await envStore.read();
      return {
        status: 200,
        body: {
          ok: true,
          hasAiKey: presence.AI_API_KEY ?? false,
