...
def run(state,action,job,options=None):
    ...
    if action=='diagnose':args+=['diagnose']
        ...
        subprocess.run(compose(state)+['restart','nocheh-app','nocheh-security','hermes'],env=compose_env(state),check=True,capture_output=True)
        subprocess.run(compose(state)+['up','-d','--no-build','--wait','--wait-timeout','180','nocheh-app','nocheh-security','hermes'],env=compose_env(state),check=True,capture_output=True)
