...
/** Buffers live group messages and flushes them into the existing processing pipeline on adjustable intervals. */
export class LiveMessageBufferService implements IncomingMessageProcessorPort {
  public constructor(
    ...
    private readonly downstream: IncomingMessageProcessorPort,
  ...
  public async flush(conversationId: string): Promise<number> {
    ...
    await this.downstream.execute({
      platform: first.platform,
      conversationId,
      messageId: `batch:${first.messageId}-${last.messageId}`,
      senderId: "nocheh-buffer",
      text: buffered.map((message) => message.text).join("\n"),
