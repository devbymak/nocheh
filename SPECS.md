...
- The owner has a separate Memory map showing people, projects, conversations, topics, versioned memory facts, descriptive relationships, project assignments, explicit access, and pending access suggestions. It uses an interactive React Flow two-dimensional clustered view positioned by the ELK layered algorithm, with draggable nodes, pan, zoom, fit controls, a minimap, deterministic reset layout, search, filters, pagination or partial-result status, keyboard navigation, reduced motion, mobile layout, and a complete non-graph list. The existing Evidence graph remains separate.
