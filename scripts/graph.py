...
CONTEXT_KINDS = ('collection','user','project','group','message')
...
def context_entities(graph):
    ...
    for source in graph.get('nodes',[]):
        ...
        kind='collection' if suffix=='*' and source.get('kind') in ('scope','group','collection') else KIND_ALIASES.get(source.get('kind'),source.get('kind'))
        if kind not in CONTEXT_KINDS:continue
        node_id=kind+':'+suffix
        ids[old_id]=node_id
        normalized={**source,'id':node_id,'kind':kind}
        if kind=='collection':normalized.pop('chat_type',None)
