...
test('real PostgreSQL: topic inheritance, revision conflicts and immutable source backfill',{skip:!process.env.PGHOST},async()=>{
  ...
  try {await initialize(pool);
    ...
    const value:Envelope={version:1,key:'space:topic',scope:'-20',bot_id:'fixture',source_id:'7',revision:'1',origin:'import',kind:'telegram_update',occurred_at:null,text:'Original',payload:{message:{message_thread_id:42}}};
