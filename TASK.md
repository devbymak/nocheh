...
controlled transition began and stopped at a fail-closed preservation check; the
...
| Running installation | The owner-approved reset stopped 13 reviewed writer/refresh containers and suppressed restart on all 17 owned containers. Four reviewed database/cache containers remain running; four inactive fences hold the installation. The journal completed `isolated_acceptance`, `quiesced`, and `effects_settled`; 65 outbound receipts reconciled with no unresolved effect. Preservation stopped before erasure because Hermes replaced two native status files during shutdown. | Integrate the scoped status-file binding fix and retry preservation under maintenance exclusion. No erasure or Telegram backlog operation started. [Settlement and fix](compatibility/results/2026-09-24-reset-post-shutdown-status-fix.json) |
| Storage transition | The original-only three-store candidate, scoped reset, and acceptance validator passed isolated rehearsals. The approved preflight found 17 owned containers, three volumes, 57 classified paths, and zero initial blockers. All 16 external items retained exact approved identities and dispositions: 13 erase installation-owned, three preserve unrelated. Quiescence and effect settlement completed; the installation remains `legacy` and inactive. The pre-erasure preservation gate found two Hermes status inodes replaced during shutdown. | Integrate the scoped rebound fix, repeat preservation, then finish erasure, initialization, and empty-baseline proof. [Plan](docs/original-only-archive-plan.md), [preflight](compatibility/results/2026-09-24-refreshed-reset-preflight.json), [status-file fix](compatibility/results/2026-09-24-reset-post-shutdown-status-fix.json) |
...
outbound receipts and no unresolved effect. Preservation found two Hermes status
files replaced during shutdown. A scoped binding fix passed 32 tests and awaits
integration and the preservation retry. No content erasure or backlog discard occurred.
...
1. Integrate the [post-shutdown status binding fix](compatibility/results/2026-09-24-reset-post-shutdown-status-fix.json), then retry the interrupted preservation gate under the [reset plan](docs/original-only-archive-plan.md). Revalidate the [private preflight and exact ownership review](compatibility/results/2026-09-24-refreshed-reset-preflight.json) under maintenance exclusion, freeze setup/credentials/accounting/preferences, perform scoped erasure and initialization, and prove the new empty baseline before resuming writers. The read-only preflight is not reset execution evidence.
