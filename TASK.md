...
| Running installation | On 2026-09-24, the owner requested an app rerun. The locally integrated `nocheh-app` and `hermes` images were rebuilt; all 17 owned services were healthy, Telegram was connected, guard mode was on, and no execution holds were present. Fresh synthetic subscription, detector, and citation checks passed. The later voice fix was activated by rebuilding only the app, Hermes, and speech services, which were healthy for the live voice checks. | Active in-flight send recovery, controlled reset, and post-reset acceptance remain pending. [First rebuild](compatibility/results/2026-09-24-main-app-rebuild.json), [voice activation](compatibility/results/2026-09-24-quiet-voice-live-activation.json) |
| Storage transition | The original-only three-store candidate, scoped reset, and acceptance validator passed isolated rehearsals. The saved installation layout is `legacy`. The latest read-only inventory found 17 owned containers, three volumes, 57 classified paths, zero ownership blockers, and 16 immediate items across five external review roots. The dashboard audio-download cache is classified for erasure. The owner approved the exact 13-erase/3-preserve proposal, conditional on the voice gate passing; the pre-reset voice condition has now passed. | Refresh the exact inventory and controlled preflight, execute the documented transition under its preservation gates, and prove the new empty baseline. This task has not run the reset. [Plan](docs/original-only-archive-plan.md), [inventory evidence](compatibility/results/2026-09-24-reset-cache-inventory.json), [voice evidence](compatibility/results/2026-09-24-exact-owner-voice-gate.json) |
...
smoke traffic; group privacy and citation passed. The quiet voice note's terminal
failure preserved its original and stopped dispatch. A fresh owner note passed the
exact-phrase subscription check. The refreshed reset inventory has no ownership
blockers; the owner approved its exact 16 external-item decisions conditional on
voice passing, and that condition is now met. The controlled reset has not run.
...
1. Refresh the exact 16-item external reset review and controlled preflight; preserve saved configuration, external logins, and spending accounting according to the [reset plan](docs/original-only-archive-plan.md). The owner approved the current decisions conditional on voice passing, and the condition is met, but no preflight or review may be reused after identity changes.
2. Run the controlled `legacy` to `original-only-v1` transition and inspect its receipt, then prove the empty baseline before resuming saved policies or writers. The read-only preflight is not reset execution evidence.
