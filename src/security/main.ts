...
const server=brokerServer({pool,token:config.token,model:process.env.NOCHEH_MODEL??'gpt-5.6-sol',
  archive:process.env.ARCHIVE_URL??'http://archive:8780',prepare:guardService(pool,config,runtimeCall(hermesAdapter({url:config.hermesUrl,token:config.token}))),hermes:config.hermesUrl});
