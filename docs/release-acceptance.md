...
| Owner retirement | Send an ordinary short owner DM, save its captured event ID and timestamp, then use Archive to retire that exact message. Verify the original and any file/transcript remain inspectable, the owner action history records the revision, the message is absent from future agent recall, and no unsent reply is delivered. Undo with the revision-checked action and verify future use is permitted. Sending a second message must leave the first decision unchanged. |
