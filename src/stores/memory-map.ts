...
export class MemoryMapRepository {
  ...
  async read(principal:Reader,input:{after:string;limit:number;focus:string;query:string;kind:string;state:string}) {
    ...
    const projects=(await this.stores.control.query('SELECT * FROM projects ORDER BY id LIMIT 5000')).rows,projectById=new Map(projects.map(row=>[row.id,row]));
    const assignments=(await this.stores.control.query('SELECT a.*,p.name FROM project_assignments a LEFT JOIN projects p ON p.id=a.project_id ORDER BY a.space_id LIMIT 5000')).rows,assignmentBySpace=new Map(assignments.map(row=>[row.space_id,row]));
    ...
    for(const row of entities){const project=row.project_id?projectById.get(row.project_id):null;nodes.push({id:`${row.kind}:${row.id}`,kind:row.kind,label:project?.name??row.name,state:project?.state??row.state,collapsed:true,
      detail:{revision:row.revision,project_id:row.project_id,fact_count:factCounts.get(row.id)??0,...(project?{project}:{}) ,...(input.focus===`${row.kind}:${row.id}`?{bindings:(await this.stores.control.query('SELECT binding_kind,label,state,source_object_id FROM memory_entity_bindings WHERE entity_id=$1 ORDER BY id',[row.id])).rows}:{})}});}
    ...
    for(const row of spaces){const kind=classify(row.id),parent=row.id.includes('/topic/')?row.id.split('/topic/')[0]:null;
      nodes.push({id:`${kind}:${row.id}`,kind,label:kind==='topic'?`Topic ${row.id.split('/topic/')[1]}`:`Conversation ${row.id}`,cluster:parent??row.id,detail:{space_id:row.id,parent,assignment:assignmentBySpace.get(row.id)??null}});}
    ...
    for(const row of claims){nodes.push({id:`fact:${row.id}`,kind:'fact',label:row.content,state:row.retired?'retired':'active',collapsed:false,
      detail:{revision:row.revision,predicate:row.predicate,relationship_kind:row.relationship_kind,attribution:row.attribution,uncertainty:row.uncertainty,retired:row.retired,evidence:row.evidence,author:row.author,created_at:row.created_at}});
      edges.push({id:`relationship:fact:${row.id}`,kind:'relationship',source:`${entities.find(entity=>entity.id===row.subject_entity_id)?.kind??'person'}:${row.subject_entity_id}`,target:`fact:${row.id}`,
        label:row.predicate,authoritative:false,detail:{claim_id:row.id,content:row.content,relationship_kind:row.relationship_kind,attribution:row.attribution,uncertainty:row.uncertainty,retired:row.retired,evidence:row.evidence,revision:row.revision,history:`/v1/entities/claims/${row.id}/history`}});
      if(row.object_entity_id)edges.push({id:`relationship:${row.id}`,kind:'relationship',source:`fact:${row.id}`,target:`${entities.find(entity=>entity.id===row.object_entity_id)?.kind??'person'}:${row.object_entity_id}`,
        label:row.relationship_kind??row.predicate,authoritative:false,detail:{claim_id:row.id,content:row.content,relationship_kind:row.relationship_kind,attribution:row.attribution,uncertainty:row.uncertainty,retired:row.retired,evidence:row.evidence,revision:row.revision,history:`/v1/entities/claims/${row.id}/history`}});}
    for(const row of assignments.filter(item=>item.mode==='assigned')){const entity=entities.find(item=>item.project_id===row.project_id);if(entity)edges.push({id:`project_assignment:${row.space_id}`,kind:'project_assignment',source:`project:${entity.id}`,target:`${classify(row.space_id)}:${row.space_id}`,label:'contains',authoritative:false,detail:{destination:row.space_id,project_id:row.project_id,mode:row.mode,revision:row.revision}});}
    ...
    const artifactIds=[...new Set([...grants.map(row=>row.representation_reference.id),...requests.map(row=>row.proposal_reference.id)])],artifacts=artifactIds.length?
      new Map((await this.stores.derived.query('SELECT id,content,provenance FROM derived_artifacts WHERE id=ANY($1::text[])',[artifactIds])).rows.map(row=>[row.id,row])):new Map<string,any>();
    for(const row of grants){const artifact=artifacts.get(row.representation_reference.id);edges.push({id:`access:${row.id}`,kind:'access',source:`fact:${row.fact_id}`,target:`${classify(row.destination)}:${row.destination}`,state:row.state,label:row.mode,authoritative:true,
      detail:{grant_id:row.id,destination:row.destination,wording:artifact?.content.toString()??null,provenance:artifact?.provenance??null,mode:row.mode,revision:row.revision,fact_revision:row.fact_revision,expires_at:row.expires_at,suspended_reason:row.suspended_reason,request_id:row.request_id,revocation:`/v1/memory-access/grants/${row.id}/revoke`}});}
    for(const row of requests){const artifact=artifacts.get(row.proposal_reference.id);edges.push({id:`suggestion:${row.id}`,kind:'suggestion',source:`fact:${row.fact_id}`,target:`${classify(row.destination)}:${row.destination}`,state:row.state,label:'suggested',authoritative:false,
      detail:{request_id:row.id,destination:row.destination,wording:artifact?.content.toString()??null,provenance:artifact?.provenance??null,revision:row.revision,fact_revision:row.fact_revision,relationship_path:row.relationship_path,evidence:row.evidence,expires_at:row.expires_at,decision:row.decision}});}
    ...
    const stateEdges=input.state?edges.filter(edge=>edge.state===input.state||edge.label===input.state):edges,stateNodes=new Set(stateEdges.flatMap(edge=>[edge.source,edge.target]));
    const unique=new Map(nodes.map(node=>[node.id,node])),filtered=[...unique.values()].filter(node=>(!input.kind||node.kind===input.kind)&&(!input.state||node.state===input.state||stateNodes.has(node.id))&&(!query||`${node.label} ${node.kind}`.toLocaleLowerCase().includes(query))).sort((a,b)=>a.id.localeCompare(b.id));
    ...
    const pageEdges=stateEdges.filter(edge=>ids.has(edge.source)||ids.has(edge.target)).sort((a,b)=>a.id.localeCompare(b.id));
