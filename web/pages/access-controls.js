...
export function createSpaceControls(React, call) {
  ...
  return function SpaceControls(){
    ...
    useEffect(()=>{const refresh=()=>setTick(v=>v+1);window.addEventListener('nocheh:refresh',refresh);return()=>window.removeEventListener('nocheh:refresh',refresh);},[]);
    ...
    useEffect(()=>{let alive=true;const controller=new AbortController();
      call('/memory/spaces?after='+encodeURIComponent(after),undefined,{signal:controller.signal}).then(result=>{if(!alive)return;
      ...
      }).catch(e=>{if(alive)setError(e.message);});return()=>{alive=false;controller.abort();};
    },[after,tick]);
    useEffect(()=>{let alive=true;const controller=new AbortController();setPolicy(null);setPreview(null);setText('');setSourceIds('');setError('');if(!space)return()=>{alive=false;controller.abort();};
      Promise.all([call('/memory/spaces?id='+encodeURIComponent(space),undefined,{signal:controller.signal}),call('/memory/shares?space='+encodeURIComponent(space),undefined,{signal:controller.signal})]).then(([p,s])=>{
        if(alive){setPolicy(p);setDraft(p.overrides);setShares(s);}
      }).catch(e=>{if(alive)setError(e.message);});return()=>{alive=false;controller.abort();};
    },[space,policyTick]);
    useEffect(()=>{let alive=true;const controller=new AbortController();
      call('/memory/reviews?after='+encodeURIComponent(jobAfter),undefined,{signal:controller.signal}).then(result=>{if(alive){setJobs(result.jobs);setJobNext(result.next);}}).catch(e=>{if(alive)setError(e.message);});
    ...
    return h('div',null,
      ...
      h(Tabs,{value:tab,onValueChange:setTab},h(TabsList,{'aria-label':'Memory access category'},...Object.entries({policy:'Access policy',shares:'Shared knowledge',recall:'Recall preview',jobs:'Learning jobs'}).map(([value,label])=>h(TabsTrigger,{key:value,value},label))),
      h(TabsContent,{value:'policy',forceMount:true},space&&!policy&&!error&&h('p',{role:'status'},'Loading space settings…'),
      ...
      policy&&!policy.private_owner&&panel('Memory access',h('p',{className:'n-muted'},policy.parent?'Inherits from '+policy.parent+'. Effective mode: '+modes[policy.effective.mode]+'.':'Application default: approved sharing.'),
      ...
      ),h(TabsContent,{value:'shares',forceMount:true},policy?.private_owner&&panel('Private owner memory',h('p',null,'Select a group or topic to approve shared knowledge.')),policy&&!policy.private_owner&&panel('Approve exact knowledge for this space',h('p',{className:'n-muted'},'Share only the text shown here. Linking an original records private provenance; it does not grant access to the complete source. Parent-group shares are inherited unless a topic uses own context only.'),
      ...
      ),h(TabsContent,{value:'recall',forceMount:true},panel('Recall and access preview',h('form',{className:'n-actions',onSubmit:e=>{e.preventDefault();run(async()=>{
        ...
        recall&&h('div',null,h('h3',null,'Owner-only results'),...[...(recall.native.hits??[]),...recall.archive].map((s,i)=>h('article',{className:'n-result',key:i},h('small',null,(s.kind||'Original record')+' · '+(s.profile||s.scope)),h('p',{dir:'auto'},s.text))),(recall.native.truncated||recall.native.next_profile)&&h('p',null,'Native results are limited. Refine your search or select a profile through the CLI.'))),
      ),h(TabsContent,{value:'jobs',forceMount:true},panel('Hermes learning jobs',h('p',{className:'n-muted'},'Imported chats enter this queue only after your approval. Running reviews finish before their worker closes. An interrupted review may require an explicit resume.'),
        button('Refresh jobs',()=>setTick(v=>v+1),busy),jobs.length===0&&h('p',null,'No review jobs on this page.'),
        ...jobs.map(j=>h('article',{className:'n-result',key:j.id},h('p',null,j.reason==='live'?'Conversation review':'Owner-approved review'),h('small',null,j.event_id+' · part '+(j.chunk_index+1)),h('p',{role:'status'},h(StatusBadge,{state:j.state,label:j.state+(j.error_code?' · '+j.error_code:'')})),
