...
test('targeted preparation and dispatch preserve source identity and exclude legacy or stale owners',{skip:!process.env.PGHOST},async()=>{
  ...
  try {
    ...
    await fetchAttachments(pool,root,download,undefined,{owner:'inngest',epoch:1});
    ...
    await prepareArchiveFiles(pool,root,async()=>{throw Error('legacy preparation cannot run');},undefined,{owner:'inngest',epoch:1});
    ...
    await prepareGuarded(pool,async()=>{throw Error('legacy guard runner cannot run');},'fixture',100,first.id,{owner:'inngest',epoch:1});
    ...
    await dispatchCommitted(pool,config,async()=>{throw Error('legacy dispatch cannot run');},undefined,{owner:'inngest',epoch:1});
