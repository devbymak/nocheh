...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='nocheh-db-fixture-') as temporary:
        ...
        override.write_text(json.dumps({'services': {'nocheh-postgres': {
        ...
        def query(database, sql):
            return subprocess.check_output(command + ['exec', '-T', 'nocheh-postgres',
        ...
        try:
            ...
            assert rendered['services']['nocheh-postgres']['image'] == image
            ...
            run('up', '-d', '--no-build', '--wait', '--wait-timeout', '180', 'nocheh-postgres')
            ...
            rejected = run('up', '-d', '--no-build', '--force-recreate', '--wait',
                           '--wait-timeout', '40', 'nocheh-postgres', check=False)
            assert rejected.returncode != 0
            values['NOCHEH_CONTROL_PASSWORD'] = control_password
            write_env(state / '.env', values)
            run('up', '-d', '--no-build', '--force-recreate', '--wait', '--wait-timeout', '180', 'nocheh-postgres')
            assert query('nocheh_control', 'SELECT generation FROM installation') == generation
