...
- The owner dashboard presents archive records in a searchable, paginated, responsive table. Its default browse and search results contain source evidence only, excluding wire captures, outbound intents/results, generated drafts, and other operational records. Incoming messages show reply-processing state separately from capture and guarded-copy readiness. Selecting a row opens its source details and the applicable revision-checked editor for guarded or derived records; immutable original evidence remains visibly read-only and is never edited as a raw database row.
...
- Distinct graph nodes do not appear as indistinguishable duplicates. Users, projects, groups, and messages have stable identities, and messages without display text identify their source kind and a stable short message ID while preserving the underlying records unchanged.
