...
def main(state,args):
    ...
    parser.add_argument('action',choices=('list','show','status','retry','cancel','pause-family','migration','reconcile','switch','host-handoff'))
    ...
    if options.action=='pause-family':
        if not options.id or not options.epoch:parser.error('Provide the family and current --epoch from workflows status.')
        identity=options.migration_id or hashlib.sha256(uuid.uuid4().bytes).hexdigest()
        print(json.dumps(API().call('/v1/workflows/migrations',{'id':identity,'family':options.id,'owner':'inngest','epoch':options.epoch}),indent=2));return 0
    if options.action in ('migration','reconcile','switch'):
