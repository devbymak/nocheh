...
export function Settings({notify}) {
 return h(Tabs,{defaultValue:'nocheh',className:'settings-page'},h(TabsList,{'aria-label':'Settings category'},h(TabsTrigger,{value:'nocheh'},'Nocheh settings'),h(TabsTrigger,{value:'hermes'},'Hermes preferences'),h(TabsTrigger,{value:'policy'},'Policy defaults')),
  h(TabsContent,{value:'nocheh',forceMount:true},h(NochehSettings,{notify})),h(TabsContent,{value:'hermes',forceMount:true},h(HermesPreferences,{notify})),h(TabsContent,{value:'policy',forceMount:true},h(PolicySettings,{notify})));
}
  function PolicySettings({notify}) {
    ...
    const save=async e=>{e.preventDefault();setBusy(true);try{await call('/policy',{changes,revision:editRevision});setChanges({});setTick(v=>v+1);notify('Global preferences saved. Profiles that inherit them use them on the next turn.');}catch(e){notify(errorText(e),true);}finally{setBusy(false);}};
  ...
  function HermesPreferences({notify}) {
    ...
    return h(Panel,{title:'Hermes preferences',note:'Agent behavior and memory limits for one chat profile. Saved in that profile’s Hermes config.yaml and used on its next turn; no service restart needed.'},
      ...
      scope&&prefs?.scope===scope&&h('form',{onSubmit:save},h('div',{className:'n-form n-preferences n-settings-preferences'},...Object.entries(prefs.schema).map(([key,spec])=>{
        ...
        return h('div',{className:'n-field n-settings-preference',key},h('label',{htmlFor:key},names[key]?.[0]||key),spec.choices?h('select',input,...spec.choices.map(v=>h('option',{key:v,value:v},v[0].toUpperCase()+v.slice(1)))):h('input',{...input,type:'number',min:spec.min,max:spec.max,required:true,step:1}),h('small',{id:key+'-help'},changes[key]===null?'Will inherit the global value after saving.':(names[key]?.[1]||'')+' Effective source: '+(prefs.origins?.[key]||'profile')),button('Inherit global value',()=>setChanges({...changes,[key]:null}),busy));
