  ...
  const server=createServer((req,res)=>{void(async()=>{
    ...
    if(req.method==='GET') {
      ...
      if(path==='/v1/data') {
        ...
        const candidates=(await s.stores.archive.query("SELECT e.id,e.channel,e.scope,e.source_id,e.revision,e.kind,e.origin,e.occurred_at,e.received_at,e.original_text,e.payload,(SELECT array_agg(a.kind ORDER BY a.id) FROM artifacts a WHERE a.event_id=e.id) AS artifact_kinds FROM events e WHERE e.id>$1 AND e.origin<>'generated' AND e.kind<>'telegram_wire' AND ($2='' OR e.scope=$2) AND ($3='' OR $3='incoming' AND e.kind IN ('telegram_update','browser_input') OR $3='assistant' AND e.kind LIKE '%_delivered_message') ORDER BY e.id LIMIT $4",[after,filters.scope,filters.kind,filters.reply?201:51])).rows;
