...
- Hermes global preference defaults appear with Hermes profile preferences in Settings. A profile preference has an accessible reset icon in its input row to remove its override and use the global value after saving. The Telegram bot token field shows whether a token is configured while keeping its value hidden. An empty selected group list explicitly says that no groups are selected and owner private messages can still work.
