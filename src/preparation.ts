...
export async function prepareArchiveFiles(pool:pg.Pool,root:string,call:RuntimeCall) {
  ...
  try {
    ...
    const {rows}=await client.query(`SELECT a.* FROM artifacts a LEFT JOIN transcription_jobs t ON t.artifact_id=a.id WHERE a.state='ready'
      ...
      AND (t.artifact_id IS NULL OR t.next_attempt<=now()) ORDER BY a.id LIMIT 10`);
    for(const row of rows) {
      if(['voice','audio','video_note'].includes(row.kind)){await prepareTranscripts(client,root,row.event_id,call);continue;}
  ...
  }finally{if(locked)await client.query('SELECT pg_advisory_unlock(hashtextextended(current_schema(),803310))');client.release();}
}
