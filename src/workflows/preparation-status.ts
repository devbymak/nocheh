...
export function waitForPreparation(prepared:Observation,attempts=0):Observation {
  if(prepared.state==='failed')return observation('failed',prepared.stage,prepared.attempts);
...
export async function preparationStatus(pool:pg.Pool,eventId:string):Promise<Observation> {
  ...
  const media=(await pool.query(`SELECT count(*)::int AS count,coalesce(max(t.attempts),0)::int AS attempts,min(t.next_attempt) AS next,
    bool_or(t.state='failed') AS failed,bool_or(t.error_code='invalid_transcription_response') AS unrecognized
  ...
  if(media.count)return observation(media.unrecognized?'failed':media.failed?'retryable_failed':'waiting','transcription',media.attempts,
    media.unrecognized?Date.now():media.next?.getTime()??Date.now(),media.unrecognized?null:media.failed?'provider_unavailable':'prerequisite');
