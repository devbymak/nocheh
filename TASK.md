...
controlled transition began and stopped at a fail-closed pre-erasure recheck; the
empty-baseline proof, fresh post-reset live checks, and post-reset Honcho production
gate have not run. Git integration and isolated tests do not complete those gates.
...
| Running installation | The owner-approved reset stopped 13 reviewed writers, suppressed all 17 restart policies, and installed four inactive fences. The journal completed `isolated_acceptance`, `quiesced`, `effects_settled`, and `preservation_frozen`; 65 outbound receipts reconciled with no unresolved effect. Four database/cache containers remained initially, and workflow Redis was then stopped under reset locks. Erasure is at `prepared` with no deletion. | Integrate the narrow stopped-Redis AOF metadata check and retry the same prepared erasure. [AOF evidence](compatibility/results/2026-09-24-reset-stopped-redis-aof-fix.json) |
| Storage transition | The original-only three-store candidate, scoped reset, and acceptance validator passed isolated rehearsals. The approved preflight found 17 owned containers, three volumes, 57 classified paths, and zero initial blockers. All 16 external items retained exact approved identities and dispositions: 13 erase installation-owned, three preserve unrelated. Preservation froze current setup, preferences, accounting, credentials, ledgers, and a 1,974-entry file manifest. One reviewed Redis AOF changed only in size/time while the cache was still running; the cache is stopped and erasure remains prepared. The installation is still `legacy` and inactive. | Integrate the stopped-cache fix, retry scoped erasure, initialize new stores, and prove empty baseline. [Plan](docs/original-only-archive-plan.md), [preflight](compatibility/results/2026-09-24-refreshed-reset-preflight.json), [AOF evidence](compatibility/results/2026-09-24-reset-stopped-redis-aof-fix.json) |
...
The owner approved the exact 16 external-item reset decisions conditional on voice
passing, and the fresh owner note passed that gate. The controlled reset began:
the journal recorded isolated acceptance, stopped 13 writers, suppressed all 17
restart policies, and installed four inactive fences. The fenced Honcho inventory
fix was integrated; quiescence and effect settlement then passed with 65 reconciled
outbound receipts and no unresolved effect. Preservation then froze setup,
preferences, accounting, credentials, ledgers, and the file manifest. A reviewed
workflow Redis AOF changed size/time before its cache was stopped; the erasure
intent remains `prepared`, with no deletion. A narrow stopped-cache fix passed
35 tests and the repeated synthetic empty-baseline rehearsal.
[Quiet-note outcome](compatibility/results/2026-09-24-quiet-voice-live-outcome.json),
[exact-phrase evidence](compatibility/results/2026-09-24-exact-owner-voice-gate.json).

1. Integrate the [stopped Redis AOF fix](compatibility/results/2026-09-24-reset-stopped-redis-aof-fix.json), then retry the prepared erasure under the [reset plan](docs/original-only-archive-plan.md). Revalidate the [private preflight and exact ownership review](compatibility/results/2026-09-24-refreshed-reset-preflight.json) under maintenance exclusion, perform scoped erasure and initialization, and prove the new empty baseline before resuming writers. The read-only preflight is not reset execution evidence.
