...
const replyState=(row:RecordRow)=>{
 ...
 const values:Record<string,{label:string;state:string;note?:string}>={done:{label:'Replied',state:'ready'},running:{label:'Processing',state:'running'},pending:{label:'Waiting',state:'queued'},failed:{label:'Retry scheduled',state:'retryable_failed',note:'Inngest will retry'},ambiguous:{label:'Delivery uncertain',state:'ambiguous',note:'Not auto-retried'},suppressed:{label:'No reply needed',state:'skipped'},cancelled:{label:'Cancelled',state:'cancelled'}};
...
export function Archive({notify}:{notify:(message:string,error?:boolean)=>void}){
 ...
 return <div>
  ...
  <section className="archive-filters" aria-label="Archive filters">
   ...
   <label>Reply status<select value={replyFilter} onChange={event=>changeFilter(setReplyFilter,event.target.value)}><option value="">All reply states</option><option value="pending">Waiting</option><option value="running">Processing</option><option value="failed">Retry scheduled</option><option value="done">Replied</option><option value="ambiguous">Delivery uncertain</option><option value="suppressed">No reply needed</option><option value="cancelled">Cancelled</option><option value="not_started">Not queued</option></select></label>
