...
controlled transition began and stopped at a fail-closed quiescence check; the
...
| Running installation | The owner-approved reset stopped 13 reviewed writer/refresh containers and suppressed restart on all 17 owned containers. Four reviewed database/cache containers remain running; four inactive fences hold the installation. The journal has only `isolated_acceptance` complete. A fenced Honcho inventory mismatch prevented the quiescence phase from completing. No erasure or Telegram backlog operation started. | Integrate the read-only inventory fix, revalidate exact identities, and retry quiescence under maintenance exclusion; then settle effects and continue the reset. [Fail-closed evidence](compatibility/results/2026-09-24-reset-fenced-inventory-fix.json) |
| Storage transition | The original-only three-store candidate, scoped reset, and acceptance validator passed isolated rehearsals. The approved preflight found 17 owned containers, three volumes, 57 classified paths, and zero initial blockers. All 16 external items retained exact approved identities and dispositions: 13 erase installation-owned, three preserve unrelated. The reset journal was created, but quiescence failed before effects settlement because fenced inventory hid five owned Honcho services; the installation remains `legacy` and inactive. | Integrate the inventory fix, repeat the failed gate, then finish preservation, scoped erasure, initialization, and empty-baseline proof. [Plan](docs/original-only-archive-plan.md), [preflight](compatibility/results/2026-09-24-refreshed-reset-preflight.json), [fenced mismatch](compatibility/results/2026-09-24-reset-fenced-inventory-fix.json) |
...
the journal recorded isolated acceptance, 13 writers were stopped, all 17 restart
policies suppressed, and four inactive fences installed. The post-shutdown inventory
rejected five owned Honcho containers because the fence hid their Compose profile.
The focused fix passed 33 tests; it must be integrated and the interrupted shutdown
retried. No content erasure or backlog discard has occurred.
...
1. Integrate the [fenced Honcho inventory fix](compatibility/results/2026-09-24-reset-fenced-inventory-fix.json), then retry the interrupted quiescence gate under the [reset plan](docs/original-only-archive-plan.md). Revalidate the [private preflight and exact ownership review](compatibility/results/2026-09-24-refreshed-reset-preflight.json) under maintenance exclusion, settle in-flight effects, freeze setup/credentials/accounting/preferences, perform scoped erasure and initialization, and prove the new empty baseline before resuming writers. The read-only preflight is not reset execution evidence.
