...
export class AttachmentRepository {
  ...
  async commit(id:string,bytes:Buffer):Promise<FileReference> {
    if(bytes.length>50*1024*1024)throw new HttpError(413,'attachment_size_limit');
  ...
  async fetch(eventId:string,fetchFile:(ref:string)=>Promise<Buffer>,authority:ExecutionAuthority):Promise<void> {
    ...
    try {
      ...
      const intake=(await client.query('SELECT transport,state FROM source_intakes WHERE event_id=$1',[eventId])).rows[0];
      if(intake?.state==='pending')return;
      ...
      for(const manifest of manifests) {
        ...
        if(manifest.file_hash) {
          const file=await this.file(manifest.id);
          try {await this.bytes(file);}catch(error) {
            if(intake?.transport!=='import'||(error as NodeJS.ErrnoException).code!=='ENOENT')throw error;
        ...
        if(intake?.transport==='import'||manifest.origin==='import'||manifest.channel!=='telegram'||manifest.source_ref.startsWith('desktop:')) {
          await client.query("UPDATE attachment_retrievals SET state='pending',error_code='import_bytes_pending' WHERE artifact_id=$1",[manifest.id]);continue;
        }
