...
type ReactionPreview={change:string;actor:string|null;target:string};
type RecordRow={id?:string;event_id?:string;derived_id?:string;scope:string;text?:string;snippet?:string;preview?:string;transcript_preview?:string|null;content_types?:string[];reaction_preview?:ReactionPreview|null;total?:number;ready?:number;channel?:string;kind?:string;received_at?:string;assistant_state?:string;assistant_stage?:string;assistant_error?:string;assistant_attempts?:number;retired?:boolean;reply_messages?:ReplyMessage[];action_review?:{id:string;state:string;count:number}};
...
const recordText=(row:RecordRow)=>row.reaction_preview?.change||[row.text,row.snippet,row.preview].find(value=>value?.trim())?.trim()||row.content_types?.map(sourceContentLabel).join(' · ')||'Message without text';
...
const recordType=(row:RecordRow)=>row.reaction_preview?'Telegram reaction':row.kind==='telegram_update'?'Incoming Telegram':row.kind==='browser_input'?'Browser message':row.kind?.endsWith('_delivered_message')?'Assistant reply':row.kind?.replaceAll('_',' ')||row.channel?.replaceAll('_',' ')||'Source record';
...
export function Archive({notify}:{notify:(message:string,error?:boolean)=>void}){
 ...
 const live=useLiveResources([path,selected?'/events/'+selected:null,selected?'/sources/'+selected+'/retirement':null,'/scopes'],selected?['/data/'+selected+'/guarded','/sources/'+selected+'/derivatives','/guards/events/'+selected,...(source.data?.artifacts||[]).map((item:{id:string})=>'/guards/artifacts/'+item.id),'/derivatives/','/guards/derived_artifacts/']:[]);
   ...
   <section className="n-panel archive-list" aria-labelledby="archive-results-title">
    ...
    {!!visible.length&&<Table aria-label="Archive records"><thead><tr><th>Message</th><th>Status</th><th>Received</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>
     {visible.map((row,index)=>{const id=row.event_id||row.id,isSelected=selected===id,ready=row.total!==undefined&&row.total>0&&row.ready===row.total,reply=replyState(row);return <tr key={id||index} className={isSelected?'selected':undefined}>
      <td><div className="archive-message-cell"><div className="archive-message-meta"><span>{recordType(row)}</span><code>{row.scope}</code>{row.total!==undefined&&<span className="archive-copy-state">Agent copy {ready?'ready':`${row.ready} / ${row.total} ready`}</span>}</div><div className="archive-record-content"><span className="archive-record-text" dir="auto" title={recordText(row)}>{recordText(row)}</span>{row.reaction_preview&&<span className="archive-reaction-context">{row.reaction_preview.actor&&<span dir="auto">By {row.reaction_preview.actor}</span>}<span>On message #{row.reaction_preview.target}</span></span>}{row.transcript_preview!==null&&row.transcript_preview!==undefined&&<div className="archive-transcript-preview"><strong>Transcript · generated</strong><span dir="auto">{row.transcript_preview||'The active transcript is empty.'}</span></div>}{!!row.content_types?.length&&!![row.text,row.snippet,row.preview].find(value=>value?.trim())&&<span className="archive-content-types"><Paperclip size={13} aria-hidden="true"/>{row.content_types.map(sourceContentLabel).join(' · ')}</span>}</div>{id&&parentByReply.has(id)&&<button className="archive-parent-link" type="button" onClick={()=>choose(parentByReply.get(id)!.id,true)}>In response to: <span dir="auto">{parentByReply.get(id)!.text}</span></button>}{!!row.reply_messages?.length&&<div className="archive-linked-replies" aria-label="Delivered replies">{row.reply_messages.map(message=><button key={message.id} type="button" className="archive-linked-reply" onClick={()=>choose(message.id,true)} aria-label={'Open delivered reply: '+replyMessageText(message)}><CornerDownRight size={14} aria-hidden="true"/><span><strong>Assistant replied</strong><span dir="auto">{replyMessageText(message)}</span></span></button>)}</div>}</div></td>
      <td title={[row.assistant_stage,row.assistant_error,row.assistant_attempts?`attempt ${row.assistant_attempts}`:''].filter(Boolean).join(' · ')||undefined}><span className="archive-reply-state">{row.retired&&<StatusBadge state="cancelled" label="Retired from Nocheh"/>}{row.action_review&&<><a href={'#activity?action='+encodeURIComponent(row.action_review.id)} className="archive-action-link"><StatusBadge state={row.action_review.state} label={actionLabels[row.action_review.state]||row.action_review.state}/></a>{row.action_review.state==='proposed'&&<small>Requested action not sent</small>}{row.action_review.count>1&&<small>{row.action_review.count} actions from this message</small>}</>}{reply&&<><StatusBadge state={reply.state} label={reply.label}/>{reply.note&&<small>{reply.note}</small>}</>}{!row.retired&&!row.action_review&&!reply&&'—'}</span></td>
