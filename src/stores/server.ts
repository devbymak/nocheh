  ...
  const server=createServer((req,res)=>{void(async()=>{
    ...
    if(req.method==='GET') {
      ...
      if(path==='/v1/data') {
        ...
        const candidates=(await s.stores.archive.query("SELECT id,channel,scope,source_id,revision,kind,origin,occurred_at,received_at,original_text FROM events e WHERE ($1='' OR (e.received_at,e.id)<(SELECT received_at,id FROM events WHERE id=$1)) AND origin<>'generated' AND kind<>'telegram_wire' AND ($2='' OR scope=$2) AND ($3='' OR $3='incoming' AND kind IN ('telegram_update','browser_input') OR $3='assistant' AND kind LIKE '%_delivered_message') ORDER BY e.received_at DESC,e.id DESC LIMIT $4",[after,filters.scope,filters.kind,filters.reply?201:51])).rows;
