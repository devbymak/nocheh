...
| Storage transition | The original-only three-store candidate, scoped reset, and acceptance validator passed isolated rehearsals. The saved installation layout is `legacy`. A fresh private read-only preflight found 17 healthy owned containers, three volumes, 57 classified paths, and zero ownership blockers. All 16 external items retained the exact approved identities and dispositions: 13 erase installation-owned, three preserve unrelated. Saved configuration, provider login, Honcho spending ledger, and subscription usage accounting have the required preservation classifications. | Execute the phased controlled reset under its preservation gates and prove the new empty baseline. Revalidate the preflight and ownership review under maintenance exclusion if the installation changes. [Plan](docs/original-only-archive-plan.md), [refreshed preflight](compatibility/results/2026-09-24-refreshed-reset-preflight.json) |
...
voice passing, and that condition is now met. A fresh preflight revalidated every
decision. The controlled reset has not run.
...
1. Run the controlled `legacy` to `original-only-v1` transition under the [reset plan](docs/original-only-archive-plan.md). Revalidate the [private preflight and exact ownership review](compatibility/results/2026-09-24-refreshed-reset-preflight.json) under maintenance exclusion, settle in-flight effects, freeze setup/credentials/accounting/preferences, perform scoped erasure and initialization, and prove the new empty baseline before resuming writers. The read-only preflight is not reset execution evidence.
