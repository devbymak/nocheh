...
test('real PostgreSQL: scheduled fire identity, missed runs, cancellation, restart and delivery approval',{skip:!process.env.PGHOST},async()=>{
  ...
  try {
    ...
    const fire=await captureInput(pool,config,source,'scheduler',{owner:'inngest',epoch:1});
    assert.equal((await captureInput(pool,config,source,'scheduler',{owner:'inngest',epoch:1})).duplicate,true);
    await assert.rejects(captureInput(pool,config,{...source,text:'changed'},'scheduler',{owner:'inngest',epoch:1}),/source_identity_conflict/);
    const claim={event_id:fire.event_id,actor:'scheduler-one',profile:'private',scope:'42'};
    await assert.rejects(claimRun(pool,config,claim,undefined,{owner:'inngest',epoch:1}),/captured_run_not_found/,'browser endpoint cannot claim a scheduled source');
    assert.equal((await claimRun(pool,config,claim,'scheduler',{owner:'inngest',epoch:1})).claimed,true);
    assert.equal((await claimRun(pool,config,claim,'scheduler',{owner:'inngest',epoch:1})).claimed,false);
    ...
    const missed=await captureInput(pool,config,{...source,id:'missed',fire_reason:'missed'},'scheduler',{owner:'inngest',epoch:1});
    assert.equal((await claimRun(pool,config,{...claim,event_id:missed.event_id},'scheduler',{owner:'inngest',epoch:1})).state,'cancelled');
    const cancel=await captureInput(pool,config,{...source,id:'cancel'},'scheduler',{owner:'inngest',epoch:1});
    await cancelScheduled(pool,{event_id:cancel.event_id});
    assert.equal((await claimRun(pool,config,{...claim,event_id:cancel.event_id},'scheduler',{owner:'inngest',epoch:1})).claimed,false);
    const active=await captureInput(pool,config,{...source,id:'active'},'scheduler',{owner:'inngest',epoch:1});
    const activeClaim={...claim,event_id:active.event_id};await claimRun(pool,config,activeClaim,'scheduler',{owner:'inngest',epoch:1});
    ...
    const restart=await captureInput(pool,config,{...source,id:'restart'},'scheduler',{owner:'inngest',epoch:1});
    await recoverScheduled(pool);assert.equal((await claimRun(pool,config,{...claim,event_id:restart.event_id},'scheduler',{owner:'inngest',epoch:1})).state,'running','restart preserves the admitted Inngest occurrence');
