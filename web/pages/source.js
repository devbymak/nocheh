...
export function Source({record,notify}){
    return h('div',null,!record.derivative_versions&&h(GuardedEditor,{key:record.id,record,call,notify}),h(Panel,{title:'Original source and files'},h(Badge,null,'Original · read only'),h('p',{dir:'auto',className:'n-source'},record.event?.text),
      button('Download source JSON',()=>exportJSON({id:record.id,reference:record.reference,event:record.event,artifacts:record.artifacts},'nocheh-source-'+record.id+'.json')),
