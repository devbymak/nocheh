...
const server=createServer((req,res)=>{void(async()=>{
 ...
 if(req.method==='GET'){
  ...
  if(route==='/graph')return json(res,200,{scope:url.searchParams.get('scope'),nodes:[{id:'group:42',kind:'group',label:'Owner private'},{id:'message:'+id,event_id:id,kind:'message',label:'Northern lights trip'},{id:'user:owner',kind:'user',label:'Owner'},{id:'project:night-sky',kind:'project',label:'Night sky'}],edges:[{from:'group:42',to:'message:'+id,kind:'contains'},{from:'user:owner',to:'message:'+id,kind:'authored'},{from:'project:night-sky',to:'group:42',kind:'project_context'}],bounds:{truncated:false},unresolved_replies:0,next:null});
