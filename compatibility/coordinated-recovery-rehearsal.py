...
import argparse,json,os,sqlite3,subprocess,sys,uuid
...
try:
    ...
    with patch.object(operations,'compose',side_effect=command),patch.object(operations,'environment',side_effect=environment):
        result=operations.backup(source,directory/'snapshot',leave_stopped=True)
        manifest=operations.validate_snapshot(directory/'snapshot')
        ...
        restored=operations.restore(directory/'snapshot',target,projects[target],18980)
    ...
    report={'passed':True,'format':6,'source_project':projects[source],'restore_project':projects[target],
        ...
        'runtime_roles_no_login':True,'provider_logins_inactive':True,'only_restored_database_running':True,'provider_calls':0,'live_state_changed':False}
