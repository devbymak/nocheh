...
class HonchoRuntimeTests(unittest.TestCase):
    ...
    def test_resolved_compose_keeps_memory_private_and_uses_preserved_external_volumes(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertEqual(set(services['database']['networks']),{'honcho-isolated'})
            self.assertNotIn('ports',services['honcho'])
            self.assertIn('honcho-memory',services['honcho']['networks']['memory']['aliases'])
            self.assertTrue(config['volumes']['honcho_redis']['external'])
            self.assertEqual(services['redis']['volumes'][0]['source'],'honcho_redis')
            for name in ('database','redis','honcho','deriver','meter'):
