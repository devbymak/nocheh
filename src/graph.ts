...
export async function evidenceGraph(pool:pg.Pool, principal:Reader, scope:string, after='', count=20, focus='') {
  ...
  if (!scope || (principal.scope!==null && principal.scope!==scope)) throw new HttpError(403,'graph_scope_denied');
  for(const id of [after,focus]) if(id && !/^[a-f0-9]{64}$/.test(id)) throw new HttpError(400,'invalid_graph_cursor');
  const {rows}=await pool.query<{id:string;source_id:string;bot_id:string;revision:string;payload:Buffer;text:string}>(
    `SELECT id,source_id,bot_id,revision,payload,left(search_text,160) AS text FROM events
     WHERE scope=$1 AND id>$2 AND ($3='' OR id=$3) ORDER BY id LIMIT $4`,[scope,after,focus,count+1]);
  const events=rows.slice(0,count).map(event=>{
    ...
    return {...event,message,namespace:JSON.stringify([event.bot_id,chat.type??'unknown',String(chat.id??scope)])};
  }), ids=events.map(e=>e.id);
  const nodes:Node[]=[{id:'scope:'+scope,kind:'scope',label:scope}],edges:Edge[]=[];
  ...
  for(const event of events){
    ...
    add({id,kind:'message',label:event.text||'(message without text)',event_id:event.id,source_id:event.source_id});link('scope:'+scope,id,'contains');
