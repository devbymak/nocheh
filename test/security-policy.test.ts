...
test('PostgreSQL security policies: owner-only revisions, deny precedence, one grant across turns, revoke before start and truthful receipts',{skip:!process.env.PGHOST},async()=>{
  ...
  try {
    ...
    assert.deepEqual(await claimControlled(pool,{actor:'worker'},undefined,{owner:'inngest',epoch:1}),{claimed:false});
    ...
    const claims=await Promise.all([claimControlled(pool,{actor:'worker-a'},undefined,{owner:'inngest',epoch:1}),claimControlled(pool,{actor:'worker-b'},undefined,{owner:'inngest',epoch:1})]);
    assert.equal(claims.filter(c=>c.claimed).length,1,'the host writer remains serialized');
    assert.equal((await pool.query('SELECT remaining FROM action_permissions WHERE id=$1',[grant.id])).rows[0].remaining,99);
    const won=claims.find(c=>c.claimed)!;assert.ok('id' in won);
    const a=await controlledAction(pool,owner,won.id);
    assert.equal((await effectLog(pool,owner,'0',a.id)).events.some(e=>e.state==='started'),false,'a claim alone is not a start');
    assert.deepEqual(await startControlled(pool,{id:a.id,actor:a.actor},{owner:'inngest',epoch:1}),{started:true});
    assert.equal((await startControlled(pool,{id:a.id,actor:a.actor},{owner:'inngest',epoch:1})).started,false,'a started operation is not replayed');
    await finishControlled(pool,{id:a.id,actor:a.actor,state:'done',result:{text:'sensitive result stays in archive only'}});
    const next=await claimControlled(pool,{actor:'worker-next'},null,{owner:'inngest',epoch:1});
    assert.equal(next.claimed,true);assert.ok('id' in next);assert.notEqual(next.id,a.id);
    const b=await controlledAction(pool,owner,next.id);
    ...
    assert.deepEqual(await startControlled(pool,{id:b.id,actor:b.actor},{owner:'inngest',epoch:1}),{started:false,reason:'permission_no_longer_valid'});
    ...
    assert.equal((await claimControlled(pool,{actor:'worker'},undefined,{owner:'inngest',epoch:1})).claimed,false,'policy deny wins over exact approval');
