    ...
    for folder in sorted(directory.iterdir()):
        ...
        with (folder/'execution.lock').open('a') as lock:
            ...
            if receipt.exists():
                ...
                if value.get('id')!=folder.name:raise ValueError('import_receipt_conflict')
                result=api.call('/v1/workflows/imports/reconcile-receipt',value)
...
def handoff(state,identity,api=None):
    ...
    if migration['to_owner']!='inngest':raise ValueError('legacy_execution_removed')
    ...
    from .tool_receipts import flush_receipts
    workflow_was_running=workflow_running(state)
    try:
