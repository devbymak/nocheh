...
export function proxyNative(req:IncomingMessage,res:ServerResponse,port:number,token:string,csrf:string) {
  ...
  const proxy=request({hostname:'127.0.0.1',port,path:nativePath(req.url??'/hermes/'),method:req.method,headers},response=>{
...
export function proxyNativeSocket(req:IncomingMessage,socket:Duplex,head:Buffer,port:number,token:string) {
  ...
  delete headers.cookie;delete headers.authorization;delete headers['x-nocheh-csrf'];
  const proxy=request({hostname:'127.0.0.1',port,path:url.pathname+url.search,headers});
