...
test('portable guarded history requires explicit restore and preserves exact owner revisions',{skip:!process.env.PGHOST},async()=>{
 ...
 try{
  ...
  await prepareGuarded(source,async text=>text.includes('mango123')?['mango123']:[]);
  ...
  await prepareGuarded(target,async()=>{throw Error('restored content must not be detected again');});
  ...
  await prepareGuarded(target,async()=>[]);
