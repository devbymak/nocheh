...
| Storage transition | The original-only three-store candidate, scoped reset, and acceptance validator passed isolated rehearsals. The saved installation layout is `legacy`. The latest read-only inventory found 17 owned containers, three volumes, 57 classified paths, zero ownership blockers, and 16 immediate items across five external review roots. The new dashboard audio-download cache is classified for erasure. An exact 13-erase/3-preserve proposal is prepared privately for owner review. | Obtain approval of the exact external-item decisions, execute the documented controlled transition only after the voice gate permits dependent release work, and prove the new empty baseline. [Plan](docs/original-only-archive-plan.md), [inventory evidence](compatibility/results/2026-09-24-reset-cache-inventory.json) |
...
| Subscription transcription | The rebuilt installation captured three owner voice notes. The first, a 2.127-second quiet recording, returned no transcript after four failed attempts. Two later notes produced subscription transcripts, guarded copies without raw masks, and confirmed replies. The owner clarified that the third note used different words, so it is not an exact-phrase accuracy failure. | Check a new normal-volume exact-phrase owner note, resolve the failed quiet note's handling, and repeat the owner voice gate after reset. [Live smoke](compatibility/results/2026-09-24-live-smoke-after-rebuild.json), [inventory follow-up](compatibility/results/2026-09-24-reset-cache-inventory.json) |
...
note used different words. A refreshed reset inventory has no ownership blockers;
the controlled reset still awaits approval of the exact 16 external-item decisions.
